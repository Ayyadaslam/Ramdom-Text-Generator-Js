...
// selecting the element or class and id
...
form.addEventListener("submit", function (e) {
  //we are disable the access  of amount defult

  e.preventDefault();

  //and we use passint   change  value  string  into number

  const value = parseInt(amount.value);
  // we use here the get value randomly in items
  ...
  // less then zero -1
  // great then > 9 item
  //awnser will be show only one paragraph

  if (isNaN(value) || value < 0 || value > 9) {
    result.innerHTML = `<p> class ="result" ${text[random]}</p>`;
    // result .inner html connect sodo paragraph  with array
  } else {
    let tempText = text.slice(0, value);
    // here we use slice methode for select to off array for 0 index to selection aarray
    tempText = tempText
      .map(function (item) {
